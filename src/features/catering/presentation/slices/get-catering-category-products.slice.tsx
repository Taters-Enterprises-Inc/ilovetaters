...
export const getCateringCategoryProducts = createAsyncThunk('getCateringCategoryProducts',
    async (param: GetCategoryProductsParam) => {
        const response : GetCategoryProductsResponse = await GetCateringCategoryProductsRepository(param);
...
export const getCateringCategoryProductsSlice = createSlice({
    ...
    extraReducers: (builder: any) => {
        builder.addCase(getCateringCategoryProducts.pending, (state: any)=>{
            state.status = GetCateringCategoryProductsState.inProgress;
        }).addCase(getCateringCategoryProducts.fulfilled, (state: any, action : PayloadAction<{message: string, data: Array<CategoryProductsModel> | null}> ) => {
            const {data, message} = action.payload;
            state.status = GetCateringCategoryProductsState.success;

            state.data = data;
            state.message = message;
        }).addCase(getCateringCategoryProducts.rejected, (state: any, action: PayloadAction<{message : string}>) => {
...
export const selectGetCateringCategoryProducts = (state : RootState) => state.getCateringCategoryProducts;
