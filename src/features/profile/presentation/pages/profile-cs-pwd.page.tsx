...
import {
  ...
  DiscountRegistrationState,
  selectdiscountRegistration,
} from "../slices/discount-registration.slice";
...
export function ProfileCsPwd() {
  ...
  const discountRegistrationState = useAppSelector(selectdiscountRegistration);
  ...
  const handleSubmitApplication = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (uploadedFile) {
      ...
      dispatch(discountRegistration(formData));
    }
  };
              ...
              <div className={`flex sm:w-[calc(50%-10px)] w-full `}>
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <DesktopDatePicker
                    ...
                    renderInput={(params) => (
                      <TextField name="birthday" fullWidth {...params} />
                    )}
                  />
