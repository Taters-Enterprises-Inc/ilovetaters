...
import { AddContactRepository, AddContactResponse, FacebookLoginPointRepository, FacebookLoginPointResponse, FacebookLoginRepository, FacebookLoginResponse, GetContactsRepository, GetContactsResponse, GetSessionRepository, GetSessionResponse} from "features/shared/data/repository/shared.repository";
...
export const getContacts = createAsyncThunk('getContacts',
    async () => {
...
export const getContactsSlice = createSlice({
    ...
    extraReducers: (builder: any) => {
        builder.addCase(getContacts.pending, (state: any)=>{
            state.status = GetContactsState.inProgress;
        }).addCase(getContacts.fulfilled, (state: any, action : PayloadAction<{message: string, data: Array<ContactModel> | undefined}>) => {
            state.data = action.payload.data;
            state.message = action.payload.message;
            state.status = GetContactsState.success;
        }).addCase(getContacts.rejected, (state: any, action : PayloadAction<{message: string}>) => {
            state.message = action.payload.message;
