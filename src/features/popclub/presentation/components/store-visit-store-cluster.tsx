...
import { selectGetStoreVisitAvailableStore } from "../slices/get-store-visit-available-stores.slice";
import { setPopClubData } from "../slices/set-popclub-data.slice";
import {
  resetStoreVisitStoreAndAddress,
  selectSetStoreVisitStoreAndAddress,
  setStoreVisitStoreAndAddress,
  SetStoreVisitStoreAndAddressState,
} from "../slices/set-store-visit-store-and-address.slice";

interface StoreVisitStoreClusterProps {
  onClose: any;
  address: string | null;
...
export function StoreVisitStoreCluster(props: StoreVisitStoreClusterProps) {
  const getStoreVisitAvailableStoreState = useAppSelector(
    selectGetStoreVisitAvailableStore
  );
  const dispatch = useAppDispatch();
  const setStoreVisitStoreAndAddressState = useAppSelector(
    selectSetStoreVisitStoreAndAddress
  );
  ...
  useEffect(() => {
    if (
      setStoreVisitStoreAndAddressState.status ===
      SetStoreVisitStoreAndAddressState.success
    ) {
      dispatch(getSession());
    }
  }, [setStoreVisitStoreAndAddressState]);
  ...
  useEffect(() => {
    if (
      ...
      dispatch(resetStoreVisitStoreAndAddress());
  ...
  return (
    <section className="text-white ">
      {getStoreVisitAvailableStoreState.search ? (
        <>
          {getStoreVisitAvailableStoreState.search.length > 0 ? (
            <section className="grid grid-cols-2 gap-1 pb-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
              {getStoreVisitAvailableStoreState.search.map((store, index) => {
        ...
        <>
          {getStoreVisitAvailableStoreState.data?.map(
            (store_cluster, index) => (
              <div key={index}>
