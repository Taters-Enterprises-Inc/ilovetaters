...
export const getAllPlatform = createAsyncThunk('getAllPlatform',
    async () => {
        const response : GetAllPlatformRepositoryResponse = await GetAllPlatformRepository();
...
export const getAllPlatformSlice = createSlice({
    ...
    extraReducers: (builder: any) => {
        builder.addCase(getAllPlatform.pending, (state: any)=>{
            state.status = GetAllPlatformState.inProgress;
        }).addCase(getAllPlatform.fulfilled, (state: any, action : PayloadAction<{message: string, data: Array<PlatformModel>}> ) => {
