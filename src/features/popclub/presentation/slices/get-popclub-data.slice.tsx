...
export const getPopClubData = createAsyncThunk('getPopClubData',
    async () => {
        const response : GetPopClubDataRepositortyResponse = await GetPopClubDataRepository();
...
export const getPopClubDataSlice = createSlice({
    ...
    extraReducers: (builder: any) => {
        builder.addCase(getPopClubData.pending, (state: any)=>{
            state.status = GetPopClubDataState.inProgress;
        }).addCase(getPopClubData.fulfilled, (state: any, action : PayloadAction<{message: string, data: PopClubDataModel}> ) => {
