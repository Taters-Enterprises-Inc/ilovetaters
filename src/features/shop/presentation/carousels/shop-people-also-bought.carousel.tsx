...
export function ShopPeopleAlsoBoughtCarousel(props: ShopPeopleAlsoBoughtCarouselProps){
    ...
    return(
                        ...
                        <div className="grid grid-cols-4 gap-4">    
                            {
                                chunk.map((product, i)=>(
                                    <button key={i} onClick={()=>{
                                        ...
                                        <img src={`https://ilovetaters.com/staging/v2/shop/assets/img/250/${product.image}`} className="rounded-t-[10px] w-full" alt="" />
        ...
        }
        {
            _3x2.map((chunk, i) => {
                ...
                return(
                    <SwiperSlide key={i} className={`hidden sm:block xl:hidden pb-2 lg:px-0`} >
                        <div className="grid grid-cols-3 gap-4">
                            {
                                chunk.map((product, i)=>(
                                    <Link key={i}  to={`/shop/products/${product.hash}`} className="bg-secondary shadow-tertiary flex flex-col shadow-md rounded-[10px] text-white h-full">
                                        <img src={`https://ilovetaters.com/staging/v2/shop/assets/img/250/${product.image}`} className="rounded-t-[10px] w-full" alt="" />
                                        <div className="p-3 space-y-2 flex flex-col justify-between flex-1">
                                            <h2 className="text-white text-sm leading-4">{product.name}</h2>
                                            <h3 className="text-white font-bold">
                ...
                return(
                    <SwiperSlide key={i} className={`sm:hidden pb-2`} >
                        <div className="grid grid-cols-2 gap-4">
                            {
                                chunk.map((product, i)=>(
                                    <Link key={i} to={`/shop/products/${product.hash}`} className="bg-secondary h-full flex flex-col shadow-tertiary shadow-md rounded-[10px] text-white ">
                                        <img src={`https://ilovetaters.com/staging/v2/shop/assets/img/250/${product.image}`} className="rounded-t-[10px] w-full" alt="" />
                                        <div className="p-3 space-y-2 flex flex-col justify-between flex-1">
                                            <h2 className="text-white text-sm leading-4">{product.name}</h2>
                                            <h3 className="text-white font-bold">
